...
inline cudaError_t NvcompTemplate<Opts_t>::CompressionH2D(const size_t &batch,
  ...
  if (error == cudaSuccess) {
    ...
    if (error == cudaSuccess) {
      ...
      error = cudaEventRecord(list_events_kernel_[id], stream_);
    }
  }
  return error;
}
...
inline cudaError_t NvcompTemplate<Opts_t>::CompressionH2H(const size_t &batch,
  ...
  cudaError_t error = cudaStreamWaitEvent(stream_, list_events_H2D_[id], 0);
  if (error == cudaSuccess) {
    error = cudaStreamWaitEvent(stream_, list_events_D2H_[id], 0);
    if (error == cudaSuccess) {
      error = static_cast<cudaError_t>(compress_asynchronously_(
          list_uncompressed_data_[id]->d_ptrs(),
          list_uncompressed_data_[id]->d_sizes(), chunk_size_, batch,
...
inline cudaError_t NvcompTemplate<Opts_t>::DecompressionD2H(
  ...
  if (error == cudaSuccess) {
    ...
    error = static_cast<cudaError_t>(decompress_asynchronously_(
        ...
        list_uncompressed_data_[id]->d_sizes(), batch, device_temporal_memory_,
        temporal_memory_size_, list_uncompressed_data_[id]->d_ptrs(), statuses_,
        stream_));
    if (error == cudaSuccess) {
      error = cudaEventRecord(list_events_kernel_[id], stream_);
    }
...
inline cudaError_t NvcompTemplate<Opts_t>::CompressionMemcpyH2D(
  ...
  cudaError_t error =
      cudaStreamWaitEvent(stream_H2D_, list_events_kernel_[id], 0);
  if (error == cudaSuccess) {
    ...
    if (error == cudaSuccess) {
      error = cudaMemcpyAsync(
          list_device_uncompressed_data_[id], *host_uncompressed_data,
          current_uncompressed_size, cudaMemcpyHostToDevice, stream_H2D_);
      if (error == cudaSuccess) {
        error = cudaEventRecord(list_events_H2D_[id], stream_H2D_);
      }
    }
  }
...
inline cudaError_t NvcompTemplate<Opts_t>::CompressionMemcpyD2H(
  ...
  cudaError_t error =
      cudaStreamWaitEvent(stream_D2H_, list_events_kernel_[id], 0);
  if (error == cudaSuccess) {
    ...
    if (error == cudaSuccess) {
      error = cudaStreamSynchronize(stream_D2H_);
      if (error == cudaSuccess) {
        ...
        error = cudaMemcpyAsync(
            ...
            cudaMemcpyDeviceToHost, stream_D2H_);
        if (error == cudaSuccess) {
          error = cudaEventRecord(list_events_D2H_[id], stream_D2H_);
        }
      }
    }
  }
...
inline cudaError_t NvcompTemplate<Opts_t>::DecompressionMemcpyH2D(
  ...
  cudaError_t error =
      cudaStreamWaitEvent(stream_H2D_, list_events_kernel_[id], 0);
  if (error == cudaSuccess) {
    error = cudaMemcpyAsync(list_device_displacements_[id],
    ...
    if (error == cudaSuccess) {
      error = cudaMemcpyAsync(
          ...
          cudaMemcpyHostToDevice, stream_H2D_);
      if (error == cudaSuccess) {
        list_compressed_data_[id]->GetNextPipeline(batch, stream_H2D_,
...
inline cudaError_t NvcompTemplate<Opts_t>::DecompressionMemcpyD2H(
  ...
  if (error == cudaSuccess) {
    size_t size = (last_copy) ? list_uncompressed_data_[id]->size(batch)
                              : batch * chunk_size_;
    error = cudaMemcpyAsync(host_decompressed_data,
                            list_device_uncompressed_data_[id], size,
                            cudaMemcpyDeviceToHost, stream_D2H_);
    if (error == cudaSuccess) {
...
inline bool NvcompTemplate<Opts_t>::InitializeCompression(
  ...
  if (error == cudaSuccess) {
    error = cudaMemsetAsync(device_last_batch_size_, 0,
                            sizeof(*device_last_batch_size_), stream_);
    if (error == cudaSuccess) {
      ...
      if (error == cudaSuccess) {
        for (size_t i_memory = 0; i_memory < number_of_auxiliar_memories_;
          ...
          list_uncompressed_data_[i_memory]->GetNextCompression(
              batch_size_, chunk_size, stream_H2D_);
          ...
          list_compressed_data_[i_memory]->InitializeCompression(
              ...
              list_device_displacements_[i_memory], stream_);
        }
        error = cudaStreamSynchronize(stream_H2D_);
      }
    }
  }
    ...
    if (error == cudaSuccess) {
      for (size_t i_memory = 0; i_memory < number_of_auxiliar_memories_;
        ...
        list_uncompressed_data_[i_memory]->InitializeDecompression(
            list_device_uncompressed_data_[i_memory], stream_);
        list_uncompressed_data_[i_memory]->GetNextDecompression(
            batch_size_, chunk_size, stream_);

        list_compressed_data_[i_memory]->InitializeDecompression(
            list_device_compressed_data_[i_memory],
            list_device_displacements_[i_memory], stream_H2D_);
      }
...
inline bool NvcompTemplate<Opts_t>::CompressDeviceToHost(
  ...
  if (result == cudaSuccess) result = cudaStreamSynchronize(stream_D2H_);
...
inline bool NvcompTemplate<Opts_t>::DecompressDeviceToHost(
  ...
  *decompressed_data_size = 0;
  ...
  if (result == cudaSuccess) {
    result = cudaStreamSynchronize(stream_D2H_);
  }
  return (result == cudaSuccess);
}
...
inline bool NvcompTemplate<Opts_t>::DecompressHostToHost(
  ...
  *decompressed_data_size = 0;
  ...
  for (uint16_t i_memory = 0; (result == cudaSuccess) && current_batch_size;
    ...
    if (result != cudaSuccess) break;
    // Stream Kernels
    result = DecompressionH2H(min_batch, i_memory);
    if (result != cudaSuccess) break;
    // Copy D2H
    result = DecompressionMemcpyD2H(
        host_decompressed_data + *decompressed_data_size,
...
bool NvcompTemplate<Opts_t>::CreateInternalStructures(const size_t &batch_size,
  ...
  if (batch_size_ != batch_size) {
    ...
    if (result = (cudaSuccess ==
      ...
      if (result = (cudaSuccess == cudaHostAlloc(&host_last_batch_size_,
                                                 sizeof(*host_last_batch_size_),
                                                 0))) {
        if (result =
                (cudaSuccess == cudaMalloc(&device_last_batch_size_,
                                           sizeof(*device_last_batch_size_)))) {
          for (size_t i_memory = 0;
               result && i_memory < number_of_auxiliar_memories_; ++i_memory) {
            result =
                (cudaSuccess ==
            ...
            list_compressed_data_[i_memory] =
                new BatchDataCompressed(batch_size_);
          }
        }
      }
    }
...
void NvcompTemplate<Opts_t>::DestroyInternalStructures() {
  if (compressed_data_) {
    ...
    cudaFree(device_last_batch_size_);
    for (size_t i_memory = 0; i_memory < number_of_auxiliar_memories_;
         ++i_memory) {
...
NvcompTemplate<Opts_t>::NvcompTemplate(
    ...
    : get_temporal_size_to_compress_(get_temporal_size_to_compress),
      ...
      device_last_batch_size_(nullptr),
  ...
  number_of_auxiliar_memories_ = 2;
