...
inline cudaError_t NvcompTemplate<Opts_t>::CompressionH2H(const size_t &batch,
                                                          const uint16_t &id) {
  cudaError_t error = cudaStreamWaitEvent(stream_kernels_[last_stream_],
    ...
    if (error == cudaSuccess) {
      ...
      if (error == cudaSuccess) {
        ...
        if (error == cudaSuccess) {
          ...
          if (error == cudaSuccess) {
            error = list_compressed_data_[id]->IncrementPipeline(
                device_last_batch_size_, batch, stream_);
            if (error == cudaSuccess) {
              error = cudaEventRecord(list_events_kernel_[id], stream_);
            }
          }
  ...
  if (error == cudaSuccess) {
    ...
    if (error == cudaSuccess) {
      error = static_cast<cudaError_t>(decompress_asynchronously_(
          ...
          device_temporal_memories_[last_stream_], temporal_memory_size_,
          list_uncompressed_data_[id]->d_ptrs(), statuses_,
          stream_kernels_[last_stream_]));
      if (error == cudaSuccess) {
        error = cudaEventRecord(list_events_kernel_[id],
                                stream_kernels_[last_stream_]);
...
bool NvcompTemplate<Opts_t>::CreateInternalStructures(const size_t &batch_size,
  ...
  bool result{true};
  if ((batch_size_ != batch_size) || (number_of_streams_ != streams)) {
    DestroyInternalStructures();
    batch_size_ = batch_size;
    number_of_streams_ = streams;
    ...
    number_of_auxiliar_memories_ = 2 * number_of_streams_;
    last_stream_ = 0;
...
NvcompTemplate<Opts_t>::NvcompTemplate(
    ...
    : get_temporal_size_to_compress_(get_temporal_size_to_compress),
      ...
      number_of_auxiliar_memories_(0),
  ...
  cudaMalloc(&device_last_batch_size_, sizeof(*device_last_batch_size_));
}
